...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-zinc-950 pb-20 sm:pb-0">
      ...
      <main className="mx-auto max-w-7xl px-3 py-4 sm:px-4 sm:py-6">
        {activeView === 'trends' ? <DailyTrendsView onOpenPriceHistory={setSelectedGameForHistory} onGoToCatalog={() => setActiveView('catalog')} /> : <div>
          <GameFilters />
          <div className="mb-4 flex items-center justify-between gap-2 text-xs text-zinc-400"><div className="flex items-center gap-2"><Gamepad2 className="h-4 w-4 text-blue-500" /><span>Gösterilen: <strong className="font-mono text-white">{games.length}</strong> / {catalogMeta.total} oyun</span>{selectedPlatform !== 'ALL' && <span className="rounded bg-zinc-800 px-2 py-0.5 font-semibold text-zinc-300">{selectedPlatform.replace('_', ' ')}</span>}</div><span className="hidden sm:block">Sayfalı katalog · hızlı yükleme</span></div>
          {isLoadingGames ? <div className="py-16 text-center text-sm text-zinc-400">CeX kataloğu yükleniyor...</div> : catalogError ? <div className="rounded-2xl border border-orange-900 bg-orange-950/30 p-6 text-center text-orange-300"><p className="text-sm font-semibold">{catalogError}</p><button onClick={() => void refreshCatalog()} className="mt-3 rounded-xl bg-zinc-800 px-4 py-2 text-xs text-white">Tekrar Dene</button></div> : games.length === 0 ? <div className="mx-auto my-8 max-w-md rounded-3xl border border-zinc-800 bg-zinc-900 p-12 text-center text-zinc-400"><FilterX className="mx-auto mb-3 h-12 w-12 text-zinc-600" /><h3 className="text-base font-bold text-white">Uygun oyun bulunamadı</h3><p className="mt-1 text-xs text-zinc-500">Filtreleri veya arama terimini değiştirin.</p></div> : <><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">{games.map((game) => <GameCard key={game.id} game={game} onOpenPriceHistory={setSelectedGameForHistory} />)}</div>{hasMoreGames && <div className="py-8 text-center"><button onClick={() => void loadMoreGames()} disabled={isLoadingMore} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-800 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-zinc-700 disabled:cursor-wait disabled:opacity-70">{isLoadingMore && <LoaderCircle className="h-4 w-4 animate-spin" />}{isLoadingMore ? 'Yükleniyor...' : 'Daha Fazla Yükle'}</button></div>}</>}
        </div>}
      </main>
      {basketCount > 0 && !isBasketOpen && <div className="fixed bottom-20 right-3 z-30 sm:bottom-6 sm:right-6 sm:hidden"><button onClick={() => setIsBasketOpen(true)} className="flex items-center gap-3 rounded-full border-2 border-white/20 bg-blue-600 px-4 py-3 font-bold text-white shadow-2xl"><ShoppingBag className="h-5 w-5" /><span className="font-mono text-sm">{formatGbp(totalBasketGbp)}</span></button></div>}
      ...
      <footer className="mt-12 border-t border-zinc-800/80 bg-zinc-950 px-4 py-8 text-xs text-zinc-500"><div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 text-center sm:flex-row sm:text-left"><div><div className="flex items-center justify-center gap-2 font-bold text-zinc-300 sm:justify-start"><span className="h-2 w-2 rounded-full bg-blue-600" />CeX UK Game Price Tracker & Travel Shopping Basket</div><p className="mt-1 text-[11px]">PS5, PS4, Xbox Series X/S, Xbox One ve Xbox 360 fiyat takibi.</p></div><div className="flex flex-wrap items-center justify-center gap-4 text-[11px]"><button onClick={() => setIsStoreGuideOpen(true)} className="underline hover:text-zinc-300">UK Mağaza Rehberi</button><button onClick={() => setIsBudgetSettingsOpen(true)} className="underline hover:text-zinc-300">Bütçe & Kur</button><a href="https://uk.webuy.com" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-400 hover:text-blue-300">uk.webuy.com Resmi Sitesi ↗</a></div></div></footer>
    </div>
  );
}
