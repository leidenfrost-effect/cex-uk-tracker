...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-zinc-950/95 backdrop-blur-md border-b border-zinc-800 text-white shadow-lg">
      {/* Top Travel Banner */}
      <div className="bg-gradient-to-r from-blue-900/60 via-zinc-900 to-blue-950/60 border-b border-blue-900/30 px-4 py-1 text-xs">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2 text-zinc-300">
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 bg-blue-600 text-white font-bold px-2 py-0.5 rounded-full text-[10px] tracking-wide uppercase">
          ...
          <div className="flex items-center gap-4 text-xs font-mono">
            ...
            >
              ...
              {!isCustomRate && exchangeRateMeta?.isStale && <span className="text-[10px] text-orange-400">(Eski)</span>}
            </button>

            <button
              onClick={onOpenStoreGuide}
              className="hidden md:flex items-center gap-1 text-zinc-400 hover:text-blue-400 transition-colors"
            >
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 cursor-pointer items-center gap-2" onClick={() => setActiveView('catalog')}>
              <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center font-black text-2xl tracking-tighter text-white shadow-lg shadow-blue-600/30 border border-blue-500">
                CeX
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="truncate font-extrabold text-base text-white tracking-tight sm:text-lg">GameTracker</span>
                  <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded bg-zinc-800 text-blue-400 border border-blue-900/40">UK</span>
                </div>
          ...
          <div className="flex-1 max-w-md hidden sm:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-zinc-900/90 border border-zinc-700/80 rounded-xl pl-10 pr-4 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
              />
              {searchQuery && (
                <button
          ...
          <div className="hidden items-center gap-2 sm:flex sm:gap-3">
            ...
            >
              <div className="relative">
                ...
                {basketCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-white text-blue-600 text-[10px] font-black rounded-full w-4 h-4 flex items-center justify-center shadow">
                    {basketCount}
                  </span>
                )}
              </div>
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:hidden">
            ...
            >
              ...
              {basketCount > 0 && (
                <span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-white px-1 text-[10px] font-black text-blue-600">
                  {basketCount}
                </span>
              )}
            </button>
        ...
        <div className="mt-2.5 sm:hidden">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-700/80 rounded-xl pl-9 pr-4 py-2 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-blue-500"
            />
          </div>
        </div>
            ...
            <div className="h-1.5 min-w-0 flex-1 max-w-xs overflow-hidden rounded-full bg-zinc-800">
              <div
                className={`h-full transition-all duration-500 ${
                  isOverBudget ? 'bg-orange-500' : budgetPercent > 80 ? 'bg-amber-500' : 'bg-emerald-500'
                }`}
                style={{ width: `${budgetPercent}%` }}
              />
            </div>
            <span className={`text-[10px] font-mono font-semibold ${isOverBudget ? 'text-orange-400' : 'text-zinc-400'}`}>
              %{budgetPercent}
            </span>
          </div>
